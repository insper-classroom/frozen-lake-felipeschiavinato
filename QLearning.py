...
class QLearning:
    ...
    def train(self, filename, plotFile):
        ...
        for i in range(1, self.episodes+1):
            ...
            while not done:
                ...
                self.q_table[state, action] = self.q_table[state, action] + self.alpha * (reward + self.gamma * next_max - old_value)
    ...
    def plotactions(self, plotFile, actions_per_episode):
        ...
        moving_average = np.convolve(actions_per_episode, weights, 'valid')
        plt.plot(moving_average)
        plt.xlabel('Episodes')
        plt.ylabel('# Actions')
        plt.title('# Actions vs Episodes')
        plt.savefig(plotFile+"_Qlearning.jpg")     
        plt.close()
